...
export async function extractAndStoreHukamnama(targetDate = new Date(), force = false) {
  ...
  if (!force && existing && imageExistsOnDisk && existing.punjabi_arth && !isExistingStaleYesterday) {
    return { status: 'already_exists', hukamnama: existing, message: 'Already synced and stored in database.' };
  }

  try {
    const titleDate = new Date(dateStr + 'T12:00:00Z').toLocaleDateString('en-US', {
      month: 'long',
      day: 'numeric',
      year: 'numeric',
    });
    const dateLineWithDay = new Date(dateStr + 'T12:00:00Z').toLocaleDateString('en-US', {
      month: 'long',
      day: 'numeric',
      year: 'numeric',
      weekday: 'long',
    ...
    if (!gurmukhiHeader) {
      try {
        ...
        if (headerNode.length) {
          gurmukhiHeader = anmolLipiToGurmukhi(headerNode.text().trim());
        }
      } catch (e) {
...
export async function forceRegeneratePoster(targetDateStr = null) {
  ...
  let punjabiDateLine = hukamnama.gurmukhi_header
