...
export function parseDisplayDate(d) {
  if (!d) return '';
  ...
  return clean.replace(/Invalid Date/gi, '').trim();
}

/**
 * Auto-generate SEO Metadata (meta_desc, meta_keywords, image_alt)
 */
export function generateHukamnamaSeoMetadata({ dateStr, titleDate, ang, raag, author }) {
  const cleanAng = ang ? String(ang).trim() : '';
  const cleanRaag = raag ? raag.replace(/\s*\([^)]*\)/g, '').split('/')[0].trim() : '';
  
  const dateFormatted = parseDisplayDate(titleDate) || parseDisplayDate(dateStr) || '';
  ...
  return { imageAlt, metaDesc, metaKeywords };
}
...
export async function getLatestHukamnama() {
  try {
    const rows = await query(
      `SELECT id, DATE_FORMAT(hukamnama_date, '%Y-%m-%d') as hukamnama_date, title, gurmukhi_header, shabad_title, ang, raag, author, content_html, gurmukhi_only, punjabi_arth, english_translation, hindi_translation, source_pdf, source_image, meta_desc, meta_keywords, image_alt, views, created_at 
       FROM hukamnamas 
...
/**
 * Fetch the last 10 Hukamnamas for the sidebar list
 */
export async function getLast5Hukamnamas(limit = 10) {
  try {
    const rows = await query(
    ...
    console.error('Error fetching recent hukamnamas:', error);
    return [];
  }
}

export const getLast10Hukamnamas = getLast5Hukamnamas;
...
export async function extractAndStoreHukamnama(targetDate = new Date(), force = false) {
  ...
  try {
      ...
      await query(
        `UPDATE hukamnamas 
         SET source_image = ?, gurmukhi_header = ?, gurmukhi_only = ?, punjabi_arth = ?, english_translation = ?, hindi_translation = ?, ang = COALESCE(?, ang), raag = COALESCE(?, raag),
             meta_desc = COALESCE(meta_desc, ?), meta_keywords = COALESCE(meta_keywords, ?), image_alt = COALESCE(image_alt, ?)
         WHERE id = ?`,
        [
    ...
    const insertSql = `
      ...
      (hukamnama_date, title, gurmukhi_header, shabad_title, ang, raag, author, content_html, gurmukhi_only, punjabi_arth, english_translation, hindi_translation, source_pdf, source_image, meta_desc, meta_keywords, image_alt)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
  ...
  try {
    await query(
      `UPDATE hukamnamas 
       ...
       WHERE id = ?`,
      [sourceImagePath, seo.metaDesc, seo.metaKeywords, seo.imageAlt, hukamnama.id]
    );
  } catch (dbErr) {
    console.warn('Could not update source_image/SEO in DB:', dbErr.message);
  }
