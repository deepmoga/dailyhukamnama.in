 ...
 */
async function generatePosterImage(dateStr, formattedDate, gurmukhiText = '') {
  ...
  const filename = `hukamnama-${dateStr}.jpg`;
  const relativePath = `/uploads/${filename}`;
  const absolutePath = path.join(uploadsDir, filename);

  // If already generated on disk, skip ("sync only one time")
  ...
  return new Promise((resolve) => {
    ...
    // Pass gurmukhi text as 3rd argument (base64-encoded to avoid shell quoting issues)
    const textB64 = Buffer.from(gurmukhiText || '', 'utf8').toString('base64');
    const cmd = `python "${scriptPath}" "${formattedDate}" "${absolutePath}" "${textB64}"`;
    ...
    exec(cmd, { timeout: 30000 }, (err, stdout, stderr) => {
      if (err) {
        ...
        console.log('Poster generated:', stdout.trim());
 ...
 * Guarantees syncing only one time per day and persisting in the database.
 */
export async function extractAndStoreHukamnama(targetDate = new Date()) {
  const dateStr = formatDate(targetDate);

  // 1. Check if already exists in database
  const existing = await getHukamnamaByDate(dateStr);
  if (existing && existing.source_image) {
  ...
  try {
    ...
    // 2. Fetch Gurmukhi header (SikhNet) — needed for poster
    ...
    // 3. Generate poster image (now that we have gurmukhiHeader)
    let sourceImagePath = existing?.source_image || null;
    if (!sourceImagePath) {
      try {
        sourceImagePath = await generatePosterImage(
          dateStr,
          `${titleDate} - ${dateLineWithDay.split(',')[0]}`,
          gurmukhiHeader || ''
        );
      } catch (err) {
    ...
    // If record exists but was missing poster — update & return
    if (existing) {
      if (sourceImagePath) {
        await query(`UPDATE hukamnamas SET source_image = ?, gurmukhi_header = ? WHERE id = ?`,
          [sourceImagePath, gurmukhiHeader || existing.gurmukhi_header, existing.id]);
        existing.source_image = sourceImagePath;
      }
      return { status: 'updated_image', hukamnama: existing };
    }

    // 4. Fetch full content from Dekho-Ji
    let contentHtml = '';
    let gurmukhiOnly = '';
    let punjabiArth = '';
    ...
    try {
      ...
      if (section.length) {
        ...
        const gurmukhiNodes = section.find('.gurmukhi, .gurbani, .bani-gurmukhi, p:not([class])');
        if (gurmukhiNodes.length) {
          gurmukhiOnly = gurmukhiNodes.map((i, el) => $(el).text().trim()).get().join('\n\n');
        }

        const punjabiNodes = section.find('.punjabi, .arth, .viakhya');
        if (punjabiNodes.length) {
          punjabiArth = punjabiNodes.map((i, el) => $(el).text().trim()).get().join('\n\n');
        }

        const englishNodes = section.find('.english, .translation');
        if (englishNodes.length) {
          englishTranslation = englishNodes.map((i, el) => $(el).text().trim()).get().join('\n\n');
        }

        const hindiNodes = section.find('.hindi');
        if (hindiNodes.length) {
          hindiTranslation = hindiNodes.map((i, el) => $(el).text().trim()).get().join('\n\n');
        }
