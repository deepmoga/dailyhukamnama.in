...
async function generatePosterImage(options, fallbackFormattedDate = '', fallbackGurmukhiText = '') {
  ...
  return new Promise((resolve) => {
    ...
    const cmd = `python "${scriptPath}" --json "${jsonB64}"`;
...
export async function extractAndStoreHukamnama(targetDate = new Date()) {
  ...
  // 1. Check if already exists in database with valid source_image and content
  const existing = await getHukamnamaByDate(dateStr);
  if (existing && existing.source_image && existing.punjabi_arth) {
  ...
  try {
    ...
    try {
      sourceImagePath = await generatePosterImage({
        ...
        englishDateStr,
        raagPunjabi: raag || '',
        raagEnglish: raag ? raag.toUpperCase() : '',
...
export async function forceRegeneratePoster(targetDateStr = null) {
  ...
  const englishDateStr = `${dateLineWithDay} (Page: ${hukamnama.ang || '724'})`;
  ...
  const sourceImagePath = await generatePosterImage({
    ...
    raagPunjabi: hukamnama.raag || '',
    raagEnglish: hukamnama.raag ? hukamnama.raag.toUpperCase() : '',
