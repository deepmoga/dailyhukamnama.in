...
export default function RichTextEditor({ value = '', onChange, placeholder = 'Start typing page description...' }) {
  ...
  const handleConfirmFileUpload = async (e) => {
    ...
    try {
      ...
      const imgHtml = `<img src="${data.url}" alt="${altToUse}" class="align-center rounded-lg shadow border border-slate-200" style="display: block; margin: 1.5rem auto; max-width: 100%; height: auto;" /><p><br/></p>`;
  ...
  const handleModalSubmit = (e) => {
    ...
    if (modalType === 'link') {
      ...
      const imgHtml = `<img src="${urlInput.trim()}" alt="${altToUse}" class="align-center rounded-lg shadow border border-slate-200" style="display: block; margin: 1.5rem auto; max-width: 100%; height: auto;" /><p><br/></p>`;
      let inserted = false;
      try {
        inserted = document.execCommand('insertHTML', false, imgHtml);
      } catch (cmdErr) {
